...
namespace BasicRPG.Allomancy
{
    ...
    /// Flare: hold Keybinds.Flare (R) while burning to "burn harder" — multipliers (strength,
    /// speed, jump, heal rate) and the reserve drain scale with Allomancer.FlareMultiplier
    /// (up to ~1.8). The Pewter Drag Crash fires after `dragCrashThreshold` seconds of continuous
    /// burn (all burn time counts toward the crash, flaring included).
    ...
    public class Pewter : MonoBehaviour
    {
        ...
        [Header("Mend (Healing)")]
        public float baseHealRate = 1f; // hp per second
        ...
        [Range(0.2f, 1f)] public float baseDamageMultiplier = 0.65f; // fraction taken
